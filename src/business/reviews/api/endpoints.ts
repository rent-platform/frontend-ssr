  ...
  endpoints: (build) => ({
    createReview: build.mutation<ReviewDTO, CreateReviewRequest>({
      ...
      invalidatesTags: (result) => [
        ...
        ...(result ? [
          { type: "Review" as const, id: `DEAL-${result.dealId}` },
          { type: "Review" as const, id: `USER-${result.reviewedUserId}` },
          { type: "Review" as const, id: `USER-SUMMARY-${result.reviewedUserId}` },
          { type: "Review" as const, id: `AD-${result.itemId}` },
          { type: "Review" as const, id: `ITEM-SUMMARY-${result.itemId}` },
        ] : []),
